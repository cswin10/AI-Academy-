...
CREATE TABLE public.quiz_attempts (
  ...
  answers_data JSONB -- Store user answers for review
);
...
CREATE TABLE public.user_activity_log (
  ...
  activity_type TEXT NOT NULL, -- 'login', 'module_complete', 'quiz_pass', 'checklist_item'
  metadata JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE public.email_signups (
  ...
  lead_magnet TEXT, -- 'starter-kit', 'roadmap-pdf', etc.
  source TEXT, -- 'homepage', 'module-page', 'popup'
...
CREATE TABLE public.cohorts (
  ...
  perks JSONB, -- ['live-support', 'certification', 'feedback-sessions']
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
BEGIN
  ...
  -- Award XP for login (small amount)
...
DECLARE
  ...
  v_xp_reward INTEGER := 500; -- Base XP for completing a module
BEGIN
  ...
  -- Award badges based on module count
  ...
  ELSIF v_module_count >= 8 THEN -- 50% of 17 modules (rounded)
    PERFORM public.award_badge_by_slug(p_user_id, 'half-way');
  ELSIF v_module_count >= 13 THEN -- 75% of 17 modules (rounded)
...
RETURNS void AS $$
DECLARE
  v_xp_reward INTEGER := 10; -- Small XP for checklist item
  v_total_completed INTEGER;
BEGIN
  -- Insert checklist progress
  ...
  IF FOUND THEN
    ...
    -- Award achievements for checklist milestones (these will auto-progress)
    -- The application layer should check and update user_achievements table
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
